  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      CREATE TABLE "pending_agent_actions" (
        ...
        "actor_type" varchar(20) NOT NULL DEFAULT 'api_key',
    ...
    `);
    await queryRunner.query(`
      CREATE UNIQUE INDEX "UQ_pending_agent_actions_idempotency"
      ON "pending_agent_actions" ("tenant_id", "idempotency_key")
      WHERE "idempotency_key" IS NOT NULL
    `);
    await queryRunner.query(`
      CREATE INDEX "IDX_pending_agent_actions_tenant_status"
      ON "pending_agent_actions" ("tenant_id", "status")
    `);
  }
