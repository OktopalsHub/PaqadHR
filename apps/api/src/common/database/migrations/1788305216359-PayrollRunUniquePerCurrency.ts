...
export class PayrollRunUniquePerCurrency1788305216359 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      ADD CONSTRAINT uk_payroll_runs_tenant_period_currency
      UNIQUE (tenant_id, period_start, period_end, base_currency)
    `);
  }
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ALTER TABLE payroll_runs
      ADD CONSTRAINT uk_payroll_runs_tenant_period
