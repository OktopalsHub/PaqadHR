...
export class PaymentMethodService {
  ...
  async findById(id: string, tenantId: string): Promise<PaymentMethod | null> {
    try {
      return this.withDecrypted(await this.repo.findOne({ where: { id, tenantId }, relations: ['member', 'member.address'] }));
    } catch (e) {
  ...
  async findPrimaryPayoutMethod(tenantId: string, memberId: string) {
    return this.payrollReadinessService.findPrimaryPayoutMethod(tenantId, memberId);
  }
