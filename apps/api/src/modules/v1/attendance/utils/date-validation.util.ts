...
export class DateValidationUtil {
  static validateMonth(month: string | number): number {
    ...
    if (Number.isNaN(monthNum) || monthNum < 1 || monthNum > 12) {
      throw new BadRequestException('Month must be a valid number between 1 and 12');
    }
    return monthNum;
  }
  ...
  static validateDateString(dateString: string): Date {
    ...
    if (dateYear < 2000 || dateYear > currentYear + 1) {
      throw new BadRequestException(`Date year must be between 2000 and ${currentYear + 1}`);
    }
    return date;
  }
  static validateMonthYear(
    ...
    const validMonth = DateValidationUtil.validateMonth(month);
    const validYear = DateValidationUtil.validateYear(year);
    const currentDate = new Date();
    const currentYear = currentDate.getFullYear();
    const currentMonth = currentDate.getMonth() + 1;
    if (validYear === currentYear && validMonth > currentMonth) {
      throw new BadRequestException('Cannot request attendance data for future months');
    }
    if (validYear > currentYear) {
      throw new BadRequestException('Cannot request attendance data for future years');
    }
