      ...
      <div className="inline-flex w-max flex-wrap items-center rounded-[8px] border border-slate-100 bg-white p-1 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] dark:border-slate-800 dark:bg-slate-950/75 dark:shadow-none">
        {PRESETS.map((item) => (
          <Button
            ...
            variant="ghost"
            className={cn(
              'rounded-[8px] px-4 text-sm shadow-none',
              preset === item.id
      ...
      {preset === 'custom' ? (
        <div className="flex flex-wrap items-center gap-3">
          <Input
            ...
            className="w-[170px] border-slate-200 bg-white text-slate-700 shadow-none focus-visible:border-transparent focus-visible:ring-2 focus-visible:ring-[#fbbf24] dark:border-slate-800 dark:bg-slate-950/60 dark:text-slate-100"
          />
